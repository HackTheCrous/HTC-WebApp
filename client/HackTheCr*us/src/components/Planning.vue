<template>
  <div id="calendar" @mousedown="unfocus">
    <div class="error" v-if="this.calendarStore.failed">
      <SettingField
        :regex="/^(https:\/\/proseconsult\.umontpellier\.fr\/)/"
        placeholder="Entre ton lien Ical"
        @send="updateIcal"
        long
      >
        <template v-slot:title> ⚠️ Tu n'as pas ajouté de lien Ical </template>
        <template v-slot:label>
          Ce lien nous permet de récupérer ton emploi du temps. Si il n’est pas
          précisé on ne pourra pas te le fournir. Pour le récupérer
          <a
            href="https://app.umontpellier.fr/prose-etudiant/protected/ical"
            target="_blank"
            >clique ici !</a
          >
        </template>
        <template v-slot:hint>
          Les liens ne commençant pas par
          <b>https://proseconsult.umontpellier.fr</b> ne sont pas accepté.
        </template>
      </SettingField>
    </div>
    <div class="border">
    ...
    <div class="day" v-for="day in this.days" :key="day.timestamp">
      <h2
        :data-day="this.getDay(day.timestamp)"
        :data-month="this.getMonth(day.timestamp)"
        ref="title"
      >
      ...
      <div
        v-for="hour in 14"
        :class="
          !this.calendarStore.isLoading(day.timestamp) ? 'hour' : 'hour loading'
        "
        :key="hour"
        :id="hour + 6"
        ref="hours"
      ...
      <DetailEventCard
        @mouseup="setfocus"
        :height="this.height * this.getHeightOfEvent(event.start, event.end)"
        :offset-y="
          this.margin + this.height * (this.getOffsetOfEvent(event.start) - 7)
        "
        v-for="event in day.data"
        :key="event.start"
      ...
      />
      <div
        v-if="this.sameDay(Date.now(), day.timestamp)"
        id="now"
        :style="{
          width: this.width + 'px',
          top:
            this.margin +
            this.height * (this.getOffsetOfEvent(Date.now()) - 7) +
            'px',
          left: new Date().getDate() * this.width,
        }"
      ></div>
    </div>
    <div class="border">
...
export default {
  ...
  methods: {
    ...
    getHeightOfEvent(timestampStart, timestampEnd) {
      return (
        this.getOffsetOfEvent(timestampEnd) -
        this.getOffsetOfEvent(timestampStart)
      );
    },
    /**
    ...
    sameDay(dateA, dateB) {
      ...
      return (
        todateA.getDate() === todateB.getDate() &&
  ...
  computed: {
    days() {
      ...
      for (
        let day = startToTimestamp;
        day < endToTimestamp + lengthDay;
        day += lengthDay
      ) {
